...
## Wrapper script for mpileup_Wunloc3.sh. ##
...
SCRIPT_PATH="/scratch/nchen11_lab/Faye/new_FSJgenome/sex_linked_scaffs/mpileup_Wunloc3.sh"
...
while IFS= read -r SAMPLE; do
  ...
  if [ -d "$SUBDIR_PATH" ]; then
    ...
    sbatch -J "${SAMPLE}" -o "${SUBDIR_PATH}/${SAMPLE}_mpileup_Wunloc2.out.txt" -e "${SUBDIR_PATH}/${SAMPLE}_mpileup_Wunloc2.err.txt" "$SCRIPT_PATH" "$SUBDIR_PATH" "$SAMPLE"
  else
