...
## Wrapper script for 04_sambamba.sh. ##
...
SCRIPT_PATH="/scratch/nchen11_lab/Faye/new_FSJgenome/sex_linked_scaffs/04_sambamba.sh"
...
while IFS= read -r SAMPLE; do
  ...
  if [ -d "$SUBDIR_PATH" ]; then
    ...
    sbatch -J "${SAMPLE}_job" -o "${SUBDIR_PATH}/${SAMPLE}_sambamba.out.txt" -e "${SUBDIR_PATH}/${SAMPLE}_sambamba.err.txt" "$SCRIPT_PATH" "$SUBDIR_PATH" "$SAMPLE"
  else
