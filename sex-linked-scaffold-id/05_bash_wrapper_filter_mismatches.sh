...
## Wrapper script for 05_filter_mismatches.sh. ##
...
SCRIPT_PATH="/scratch/nchen11_lab/Faye/new_FSJgenome/sex_linked_scaffs/05_filter_mismatches.sh"
...
while IFS= read -r SAMPLE; do
  ...
  if [ -d "$SUBDIR_PATH" ]; then
    ...
    sbatch -J "${SAMPLE}_job" -o "${SUBDIR_PATH}/${SAMPLE}_filter.mismatches.out.txt" -e "${SUBDIR_PATH}/${SAMPLE}_filter.mismatches.err.txt" "$SCRIPT_PATH" "$SUBDIR_PATH" "$SAMPLE"
  else
