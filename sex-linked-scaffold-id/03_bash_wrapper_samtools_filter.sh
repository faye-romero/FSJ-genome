...
## Wrapper script for 03_samtools_filter. ##
...
SCRIPT_PATH="/scratch/nchen11_lab/Faye/new_FSJgenome/sex_linked_scaffs/03_samtools_filter.sh"
...
while IFS= read -r SAMPLE; do
  ...
  if [ -d "$SUBDIR_PATH" ]; then
    ...
    sbatch -J "${SAMPLE}_job" -o "${SUBDIR_PATH}/${SAMPLE}_samfilter.out.txt" -e "${SUBDIR_PATH}/${SAMPLE}_samfilter.err.txt" "$SCRIPT_PATH" "$SUBDIR_PATH" "$SAMPLE"
  else
